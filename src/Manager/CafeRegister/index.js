...
const CafeRegister = () => {
  ...
  const [cafeImages, setCafeImages] = useState({ titleFile: null, detailFiles: [] });
  ...
  const addNumber = () => {
    ...
    if (number === 2 && !cafeType) {
     ...
     // CafeBasic 유효성 검사
     if (number === 3) {
      if (!cafeBasicInfo.address || !cafeBasicInfo.phoneNumber || cafeBasicInfo.startTime >= cafeBasicInfo.endTime) {
  ...
  const handleBasicInfoChange = useCallback((basicInfo) => {
  ...
  const handleFinalSubmit = async () => {
    ...
    formData.append("cafeReg", new Blob([JSON.stringify({
      cafeName,
      cafeType,
      startTime: cafeBasicInfo.startTime.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
      endTime: cafeBasicInfo.endTime.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
    ...
    cafeImages.detailFiles.forEach((file, index) => {
      formData.append(`cafeImgs[${index}]`, file);
    });
    ...
    formData.append("cafeFeature",new Blob([JSON.stringify(cafeFeatures)] ,{'Content-Type': 'application/json'}));
      ...
      <div className="register-full-container">
        ...
        <div className="button-container">
          {/* <div className="register-buttons"> */}
            ...
            {number === 1 && <CafeName cafeName={cafeName} setCafeName={setCafeName} />}
            {number === 2 && <CafeType cafeType={cafeType} setCafeType={setCafeType} />}
            {number === 3 && <CafeBasic cafeBasicInfo={cafeBasicInfo} onBasicInfoChange={handleBasicInfoChange} />}
              {number === 4 && <CafeImage onImageChange={handleImageChange} />}
              {number === 5 && <CafeFeature onFeaturesChange={handleFeaturesChange} />}
              {number === 6 && <CafeStudySetting onStudySettingChange={handleStudySettingChange} onFinalSubmit={handleFinalSubmit} />}
            </div>
